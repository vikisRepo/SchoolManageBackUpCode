...
namespace SMS.Controllers
{
  ...
  public class LessonPlanController : ControllerBase
  {
    ...
    public void Post([FromBody] LessonPlanRequest lessonPlan)
    {
      ...
      var academicClassDetails = _dbcontext.AcademicClasses.Where(x => x.ClassName == lessonPlan.ClassName).FirstOrDefault();
      List<string> academicclass = academicClassDetails.AcademicClassSubjectId.Split(',').ToList();

      LessonPlan _updLessonPlan = new LessonPlan();

      if (academicclass.Contains(lessonPlan.SubjectName))
      {

        _updLessonPlan.AcademicClassId = academicClassDetails.AcademicClassId;
        _updLessonPlan.Subject = lessonPlan.SubjectName;
